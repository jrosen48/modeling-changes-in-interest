...
d_for_m2 <- left_join(d_ind_level_2, d_BLUP, by = "participant_ID") # 203 obs
d_for_m2_filtered <- filter(d_for_m2, !is.na(post_interest) & !is.na(gender_female)) # only 141
```
...
*Note*: Make it so square root is automatically calculated (that function takes awhile to run & is why it's not run quite yet).
...
RM engagement seems important; pre-interest seems important; gender female not quite important. Effect of RM engagement is smaller than estimated with BLUP.
