...
```{r, model0b, eval = TRUE}
ds <- filter(d, !is.na(pre_interest) & !is.na(gender_female))
m0bi <- lmer(rm_engagement ~ 1  + pre_interest + gender_female + (1 | participant_ID), data = ds)

broom::tidy(m0bi) %>% clipr::write_clip()
...
m0bii <- lm(post_interest ~ 1 + rm_engagement_BLUP + gender_female + pre_interest, data = d_for_m2) 
broom::tidy(m0bii) %>% clipr::write_clip()
```

```{r, m0b-res, cache = FALSE, eval = TRUE}
summary(m0bi)
m0bi %>% broom::tidy() %>% clipr::write_clip()
r2glmm::r2beta(m0bi)
summary(m0bii)
m0bii %>% broom::tidy() %>% clipr::write_clip()
r2glmm::r2beta(m0bii)
sqrt(.116)
```
...
d_red <- ungroup(d_red)
d_red$program_ID <- as.factor(d_red$program_ID)
levels(d_red$program_ID) <- letters[1:9]

m2 <- MCMCglmm(fixed = cbind(rm_engagement, post_interest) ~ 
                   -1 + trait:gender_female + trait:pre_interest,
               random=~us(trait):participant_ID,
               #     us(trait):program_ID, 
               rcov=~us(trait):units, # this denotes response vars varying across response rows
               family = rep("gaussian",2),
               data=as.data.frame(d_red),
               prior=prior2,
               verbose=FALSE)
