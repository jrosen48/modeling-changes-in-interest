...
m2 <- MCMCglmm(fixed = cbind(rm_engagement, post_interest) ~ trait + 
                 trait:gender_female + 
                 trait:pre_interest,
               random =~ us(trait):participant_ID + idh(trait):program_ID,
               rcov = ~ us(trait):units,
...
# m2i_cor <- m2$VCV[,"traitpost_interest:traitrm_engagement.program_ID"]/
#   (sqrt(m2$VCV[,"traitpost_interest:traitpost_interest.program_ID"])*
#      sqrt(m2$VCV[,"traitrm_engagement:traitrm_engagement.program_ID"]))
# posterior.mode(m2i_cor)
# plot(m2i_cor)

plot(m2$VCV[,"traitrm_engagement:traitrm_engagement.participant_ID"]/(m2$VCV[,"traitrm_engagement:traitrm_engagement.participant_ID"] + m2$VCV[,"traitrm_engagement.units"]))

program_icc <- (m2$VCV[,"traitrm_engagement.program_ID"]/(m2$VCV[,"traitrm_engagement.program_ID"] + m2$VCV[,"traitrm_engagement.units"]))
posterior.mode(program_icc)

post_int_icc <- (m2$VCV[,"traitpost_interest.program_ID"]/(m2$VCV[,"traitpost_interest.program_ID"] + m2$VCV[,"traitpost_interest.program_ID"]))
plot(post_int_icc)
posterior.mode(post_int_icc)
```

```{r, icc}
# rep4<-(m2$VCV[,"traitpost_interest:traitpost_interest.participant_ID"]/ (m2$VCV[,"traitpost_interest:traitpost_interest.program_ID"]+m2$VCV[,"traitrm_engagement.units"]))
# posterior.mode(rep4)
# HPDinterval(rep4)
var(m2$VCV[, 'traitpost_interest.program_ID'])
var(m2$VCV[, 'traitrm_engagement.program_ID'])
var(m2$VCV[, 'traitrm_engagement:traitrm_engagement.participant_ID'])
var(m2$VCV[, 'traitpost_interest:traitpost_interest.participant_ID'])
```
...
m3 <- brm(mvbind(rm_engagement, post_interest) ~ 
            ...
            (1|p|participant_ID) +
            (1|q|program_ID),
          data = d_for_m2, chains = 3, cores = 3, iter = 750,
